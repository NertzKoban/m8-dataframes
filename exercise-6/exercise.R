...
MostPopular <- function(my.year) {
  prop.year <- female.names$prop[year == my.year]
  name.year <- female.names$name[year == my.year]
  return(paste(name.year[prop.year == max(prop.year)], "was the most popular name in ", my.year))
  }
...
HowPopular <- function(my.name, my.year) {
  prop.year <- female.names$prop[year == my.year]
  name.year <- female.names$name[year == my.year]
  prop.name <- prop.year[name.year == my.name]
  return(paste(my.name, "had a frequency of ", prop.name, " during the year ", my.year))
}
